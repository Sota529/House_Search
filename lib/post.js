...
export async function getFavoriteData(Area, Sort) {
  const userFavo = db.collection("houses").doc("");
  const snapshot = await userFavo.get();
  const result = snapshot.data().favo;
